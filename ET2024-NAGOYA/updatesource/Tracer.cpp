...
void Tracer::changePID(int8_t& leftPower, int8_t& rightPower, int8_t maxPower, const rgb_raw_t& rawColor){
    int brightness = getBrightness2(rawColor);
    ...
    if(brightness > blueThreshold){
        ...
        rightPower = maxPower;
        leftPower = 0;
    } else if(grayBrightness < brightness){
        double diff = brightness - grayBrightness;
        // 白フロア上にいる
        leftPower = (double)maxPower * (1.0 - diff / COLOR_WIDTH);
        rightPower = maxPower;
...
Tracer::changemoveRoboPID(RoboBody& robo, int8_t maxPower){
    ...
    int8_t leftPower, rightPower;
    changePID(leftPower,rightPower,maxPower,rawColor);
    robo.setPower(leftPower, rightPower);
}
